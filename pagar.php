...
if(!isset($_POST['submit'])) { //Para que al acceder a la pagina copiando y pegando el link, no regrese valor de 0 y muestre mensaje de error
  exit("Hubo un error");
}
...
if(isset($_POST['submit'])):  //Función isset va a revisar que la variable exista. nombre submit en el btnRegistro
  $nombre = $_POST['nombre'];
  ...
  $numero_boletos = $boletos; //Creamos copia de boletos
  ...
  try {
    ...
    $stmt->close();
    $conn->close();
    /*header('Location: validar_registro.php?exitoso=1'); //Nos aseguramos que no sea reingresado el usuario si volvemos a recargar la pagina, cargamos toda la logica antes de header*/
  } catch (Exception $e) {
...
$compra = new Payer(); //Creamos clase para crear pago
...
foreach($numero_boletos as $key => $value) { //Para crear articulos
...
foreach($pedidoExtra  as $key => $value) { //Para crear articulos
  if( (int) $value['cantidad'] > 0) {
...
$cantidad->setCurrency('USD')
         ->setTotal($total) //Estar pendiente, en video ponen precio
         ->setDetails($detalles);
