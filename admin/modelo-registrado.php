...
$pedido = productos_json($boletos_adquiridos, $camisas, $etiquetas); //Debe ser en el mismo orden que se muestra en funciones
...
if($_POST['registro'] == 'actualizar') {
  ...
  try {
    $stmt = $conn->prepare('UPDATE registrados SET nombre_registrado = ?, apellido_registrado = ?, email_registrado = ?,
      fecha_registro = ?, pases_articulos = ?, talleres_registrados = ?, regalo = ?, total_pagado = ?, pagado = 1 WHERE ID_Registrado = ? ');
